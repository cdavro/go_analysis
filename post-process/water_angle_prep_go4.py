...
for f in list_filename:
    ...
    if (os.path.isfile(filename)):
        ...
        try:
            ...
            WA_AS_go4 = np.zeros((1,WA_import.shape[1]))
        WA_AS_go4 = np.append(WA_AS_go4, WA_import, axis=0)
        del(WA_import)
...
print("Write go4_WA_AS.npz Timings: ", timedelta(seconds=timer()-start))
...
for f in list_filename:
    ...
    if (os.path.isfile(filename)):
        ...
        try:
            ...
            WA_IS_go4 = np.zeros((1,WA_import.shape[1]))
        WA_IS_go4 = np.append(WA_IS_go4, WA_import, axis=0)
        del(WA_import)
...
print("Write go4_WA_IS.npz Timings: ", timedelta(seconds=timer()-start))
