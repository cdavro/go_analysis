        ...
        IF (iostatus == 0) then
            ...
            SELECT CASE (label)
                CASE ('file_pos')
                    ...
                    PRINT'(A50,A64)', 'XYZ positions file:', file_pos
                CASE ('file_vel')
                    READ(value, *, IOSTAT=iostatus) file_vel
                    PRINT'(A50,A64)', 'XYZ velocities file:', file_vel
                CASE ('file_surf')
                    READ(value, *, IOSTAT=iostatus) file_surf
                    PRINT'(A50,A64)', 'XYZ velocities file:', file_surf
                CASE ('nb_atm')
                ...
                CASE ('dens_dr')
                    ...
                    PRINT *, 'dens_dr: ', dens_dr
                CASE ('dens_step')
                    READ(value, *, IOSTAT=iostatus) dens_step
                    PRINT *, 'dens_step: ', dens_step
                CASE ('dens_rstart')
                ...
                CASE DEFAULT
                    PRINT'(A50,A64)', 'Invalid label, line:', line_c
            END SELECT
        END IF
    END DO RL
    PRINT'(A640)','--------------------------------------------------'&
